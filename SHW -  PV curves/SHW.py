...
bui = ['Waterfront Appartementen CME',
       'Waterfront CME']

# params=[
#         # 'T Depart (SHW Circuits)',
...
#           # 'T Retour (SHW Circuits)',
          
#         ]


instancesCME = ['SWW Totaal Verbruik', 'Productie WKK', 'Productie WKK']
paramsCME = ['Volume (Watercounter)', 
             # 'Volume Flow (Heat)', 
             # 'Volume (Heat)'
             ]

instancesAp = ['AS 35 11', 'AS 35 12', 'AS 35 13', 'AS 35 14', 'AS 35 15', 'AS 35 21', 'AS 35 22', 'AS 35 23', 'AS 35 24', 'AS 35 25', 'AS 35 31', 'AS 35 32', 'AS 35 33', 'AS 35 34', 'AS 35 35', 'AS 35 41', 'AS 35 42', 'AS 35 43', 'AS 35 44', 'AS 35 45', 'AS 35 51', 'AS 35 52', 'AS 35 53', 'AS 35 54', 'AS 35 55', 'AS 35 56', 'AS 35 61', 'AS 35 62', 'AS 35 63', 'AS 35 64', 'AS 35 65', 'AS 35 66', 'AS 35 71', 'AS 35 72', 'AS 35 73', 'AS 35 74', 'AS 35 75', 'AS 35 76', 'AS 35 81', 'AS 35 82', 'AS 35 82 AC', 'AS 35 83', 'AS 35 84', 'AS 35 85', 'AS 35 91', 'AS 35 92', 'AS 35 93', 'AS 35 94', 'AS 35 95', 'AS 35 101', 'AS 35 102', 'AS 35 102 AC', 'AS 35 103', 'AS 35 104', 'AS 35 105', 'AS 35 111', 'AS 35 113', 'AS 35 114', 'AS 35 115', 'AS 35 121',
               'AS 35 121 CV', 'AS 35 122', 'WI 37 11', 'WI 37 12', 'WI 37 13', 'WI 37 14', 'WI 37 15', 'WI 37 21', 'WI 37 22', 'WI 37 24', 'WI 37 25', 'WI 37 31', 'WI 37 32', 'WI 37 33', 'WI 37 34', 'WI 37 35', 'WI 37 41', 'WI 37 42', 'WI 37 43', 'WI 37 44', 'WI 37 51', 'WI 37 52', 'WI 37 53', 'WI 37 54', 'WI 37 55', 'WI 37 56', 'WI 37 61', 'WI 37 62', 'WI 37 63', 'WI 37 64', 'WI 37 65', 'WI 37 66', 'WI 37 71', 'WI 37 72', 'WI 37 73', 'WI 37 74', 'WI 37 75', 'WI 37 76', 'WI 37 81', 'WI 37 82', 'WI 37 83', 'WI 37 84', 'WI 37 85', 'WI 37 85 AC', 'WI 37 91', 'WI 37 92', 'WI 37 93', 'WI 37 93 AC', 'WI 37 94', 'WI 37 95', 'WI 37 95 AC', 'WI 37 101', 'WI 37 102', 'WI 37 103', 'WI 37 104', 'WI 37 105', 'WI 37 111', 'WI 37 112', 'WI 37 113', 'WI 37 114', 'WI  37 25 AC',]
paramsCMEap = [
    # 'Volume (Heat)', 
    # 'Volume Flow (Heat)', 
...
# paramIns = [[i, j] for i in paramsCME for j in instancesCME] + [[i, j] for i in paramsCMEap for j in instancesAp] 

paramIns = [[i, instancesCME] for i in paramsCME] +  [[i, instancesAp[10*j:10*(1+j)]] for i in paramsCMEap for j in list(range(11))]

# b = [[i, instancesAp[10*j:10*(1+j)]] for i in paramsCMEap for j in list(range(11))]

stop = int(datetime.datetime.now().timestamp()*1000)
start = int(datetime.datetime.strptime('2022-08-20', "%Y-%m-%d").timestamp()*1000)
...
#                     'Trend.Status': "status",
#                     'Trend.Time': 'time',
#                     'Trend.AverageValue': "ave",
#                     'Trend.MinimumValue': "min",
#                     'Trend.MaximumValue': "max",
#                     'E': 'bui', 
#                     'P': 'param', 
#                     }, inplace=True)            
...
# dfj.drop(['Time', 'AverageValueString', 'EPI', 'offset', 'PI', 'EI', 'EP', 'past'], axis=1, inplace=True)
# dfj['status'] = dfj['status'].astype('int')

dfj['ave'] = np.where(dfj['Status'].astype(int) < 0, np.nan, dfj['AverageValue'])
# dfj['min'] = np.where(dfj['status'] < 0, np.nan, dfj['min'])
# dfj['max'] = np.where(dfj['status'] < 0, np.nan, dfj['max'])
# dfj['range'] = dfj['max']-dfj['min']
dfj.sort_values(by=['E', 'P', 'Time'], ascending=[True, True, True], inplace=True)

# dfj.to_excel(r'C:/Users/jborr/OneDrive/Noven/calorimeters.xlsx')
# dfj.to_pickle(r'C:/Users/jborr/OneDrive/Noven/SHW/Waterfront/waterfront.pk')
# dfj.to_csv(r'C:/Users/jborr/OneDrive/Noven/23.09.csv')
...
# dfj1 = pd.read_pickle(r'C:\Users\jborr\OneDrive\Noven/23.08.pk')
# dfj2 = pd.read_pickle(r'C:\Users\jborr\OneDrive\Noven/23.09.pk')

# dfj = pd.concat([dfj1,dfj2])
...
#%% ⭐⭐📊 Jonas Seys how much time the flow is 0 m³ per day?    
import plotly.express as px
import plotly
# import chart_studio.plotly as py


# pd.read_pickle(r'C:/Users/jborr/OneDrive/Noven/SHW/Waterfront/waterfront.pk')
dfj2 = pd.read_pickle(r'C:/Users/jborr/OneDrive/Noven/SHW/Waterfront/waterfront.pk')
p = [
     # 'Volume (Heat)', 
     # 'Volume Flow (Heat)', 
     'Volume SWW (Watercounter)',
...
dfNew = dfNew.groupby(by='EPI').resample('5min').mean(numeric_only=True).ffill()
allAps = dfNew.reset_index()
allAps = allAps[allAps['EPI'].str.contains('Appartementen')].groupby('t')['ave'].sum()
allAps = pd.DataFrame(allAps)
allAps['t'] = allAps.index
allAps['EPI'] = 'allAps'
allAps = allAps.set_index([allAps['EPI'], allAps['t']])
dfNew = pd.concat([dfNew,allAps[['ave']]], axis = 0)

dfNew['diff'] = dfNew.groupby(by='EPI')['ave'].diff()
dfNew = dfNew.reset_index()
dfNew['date'] = dfNew['t'].dt.date
limit = 0.0005
dfNew['zero'] = np.where(dfNew['diff'] <= limit, 1, 0)
name = f'Minutes per day [min/day]'

dfNew[name] =  dfNew.groupby(by=['EPI', 'date'])['zero'].transform('sum')*5

allaps = dfNew[dfNew['EPI'].str.contains('Appartementen')]

# dfNew[dfNew['EPI'].str.contains('Waterfront CME')].groupby(by='EPI')['diff'].plot()
# dfNew[dfNew['EPI'] == 'Waterfront CME, Volume (Watercounter) (SWW Totaal Verbruik)'].groupby(by='date')['zero'].sum().plot()
# dfNew[dfNew['EPI'] == 'Waterfront CME, Volume (Watercounter) (SWW Totaal Verbruik)'].groupby(by='date')[name].plot()
# dfNew[dfNew['EPI'] == 'Waterfront Appartementen CME, Volume SWW (Watercounter) (WI 37 93 AC)'].groupby(by='date')[name].plot()
res = '5-60'
title = f'Minutes per day below <b>{limit} m³/h - Based on volume (not flow)</b><br>resolution: <b>{res} minutes</b> - <i>1 day = 1440 minutes'
dfNew.drop_duplicates(subset=['EPI', 'date'], inplace=True)
dfFig = dfNew[dfNew[name] < 14400]
order = dfFig.groupby('EPI')[name].median().sort_values().index
...
for i in list(range(1,25)):
    t = "hour" if i == 1 else "hours"
    fig.add_hline(y=i*60, 
                  line_dash="dot",
                  line_width=1,
                  line_color = 'crimson',
                  annotation_text=f"{i} {t}", 
                  annotation_position="top right")
    
for i in fig.data:    
    if i.legendgroup == 'ALL BUILDINGS':
        # print(i.legendgroup )
        i.marker.color = 'black'
    

fig.add_layout_image(
    dict(
        # source="https://noven.be/wp-content/uploads/2021/08/noven.png",
        source = 'https://1drv.ms/i/s!Autijg-0IF0ojaQeYLve1kr2FVIddQ?e=ewlwuV',
...
fig.layout.yaxis.range = [-5,1500]
fig.layout.xaxis.range = [-0.36,0.39]
# py.iplot(fig, fileopt='overwrite', filename='Logos/Noven')

fig.show('browser')
# fig.write_html(fr'C:/Users/jborr/OneDrive/Noven/SHW/Waterfront/Jonas SHW zero flow per day (Res. {res} minutes).html')
# dfFig.groupby('EPI').describe().to_excel(fr'C:/Users/jborr/OneDrive/Noven/SHW/Waterfront/')
# df2.groupby('bui')[['ave', name]].describe().to_excel(fr'C:/Users/jborr/OneDrive/Noven/SHW/Jonas SHW zero flow per day (Res. {res} minutes).xlsx')


#%% ⏱️Johan flow vs hour pof the day
import plotly.express as px
import plotly
# import chart_studio.plotly as py
...
allAps = pd.DataFrame(allAps)
allAps['t'] = allAps.index
# allAps['hour'] = allAps.index.hour
allAps['EPI'] = 'allAps'
allAps = allAps.set_index([allAps['EPI'], allAps['t']])
dfNew = pd.concat([dfNew,allAps[['ave']]], axis = 0)
dfNew['diff'] = dfNew.groupby(by='EPI')['ave'].diff().clip(lower=0, upper=5)    
dfNew = dfNew.reset_index()
dfNew['date'] = dfNew['t'].dt.date
dfNew['hour'] = dfNew['t'].dt.hour
# dfNew.groupby(by='EPI')['hour'].hist(bins = 24)
...
title = f'Minutes per day below <b>{limit} m³/h - Based on volume (not flow)</b><br>resolution: <b>{res} minutes</b> - <i>1 day = 1440 minutes'
# dfNew.drop_duplicates(subset=['EPI', 'date'], inplace=True)
dfFig = dfNew[(dfNew['diff']>0)
              & ((dfNew['EPI'].str.contains('allAps') | (dfNew['EPI'].str.contains('Waterfront CME'))))
              ].sort_values(by='hour')
order = dfFig.groupby('EPI')[name].median().sort_values().index
#%%% 👎CME vs allAps
dfFig = dfNew[(dfNew['diff']>0)
...
dfFig['Flow [m³/h]'] = dfFig['diff']*12 # from m³/5min to m³/h

dfFig = dfFig.pivot(index='t', columns='EPI', values='Flow [m³/h]')

fig = px.scatter(dfFig,
                x = 'Waterfront CME, Volume (Watercounter) (SWW Totaal Verbruik)',
                y = 'allAps',
                # color = 'EPI',
                # points = 'all',
                # add a regression line
                trendline = 'lowess',
                title = '<b>🚿Waterfront: CME vs allAps</b>'+
                f'<br>{dfFig.shape[0]} datapoints - {dfFig.index.date.min()} to {dfFig.index.date.max()}',
...
# add a text next to the line, and position it in the middle of the line
fig.add_annotation(
            x=1.5,
            y=1.5,
            text="y = x",
...
fig.update_yaxes(title_text='allAps Flow [m³/h]', tick0=0, dtick=0.2, ticksuffix = ' m³/h')
fig.update_xaxes(title_text='CME Flow [m³/h]', tick0=0, dtick=0.2, ticksuffix = ' m³/h')
fig.write_html(fr'C:/Users/jborr/OneDrive/Noven/SHW/Waterfront/CME vs allAps.html')
fig.show('browser')
#%%% 📊boxplot
dfFig = dfNew[(dfNew['diff']>0)
              # & (dfNew['diff']<0.4)
...
fig = px.box(dfFig,
                ...
                color = 'EPI',
                # points = 'all',
                title = '<b>🚿Waterfront: SHW Flow above 0 m³/hour</b><br><i>How is the flow distributed in the day?'+
                ' Be careful, not all hours have the same amount of points (see histograms)<br>'+
                f'{dfFig.shape[0]} datapoints - {dfFig["t"].dt.date.min()} to {dfFig["t"].dt.date.max()}',
                # marginal_x = 'histogram',
                color_discrete_sequence=['crimson', 'grey'],
                )

# fig.layout.yaxis.range = [-5,1500]
#  Add units m³/h tot he y axis, and add ticks every 0.1 m³/h
fig.update_yaxes(title_text='Flow [m³/h]', tick0=0, dtick=0.1, ticksuffix = ' m³/h')
fig.layout.yaxis.range = [-0.01,2.5]
fig.update_xaxes(
    tickmode = 'array',
...
fig.write_html(fr'C:/Users/jborr/OneDrive/Noven/SHW/Waterfront/Johan boxplot.html')
fig.show('browser')

# fig = px.density_heatmap(dfFig[dfFig['diff']<0.4],
#                 x = 'hour',
#                 y = 'diff',                
#                 # color = 'EPI',
#                 title = 'Average flow per hour of the day',                
#                 marginal_x = 'histogram',
...
fig = px.histogram(dfFig,
                x = 'hour',
                # y = 'diff', 
                color = 'EPI',
                histnorm = 'probability',
                barmode = 'overlay',
                title = '<b>🚿Waterfront: SHW Flow above 0 m³/hour</b><br><i>Taking a random non-null flow, what are the chances of belonging to a specific hour?'+
                f'<br>{dfFig.shape[0]} datapoints - {dfFig["t"].dt.date.min()} to {dfFig["t"].dt.date.max()}',
                marginal = 'box',
                color_discrete_sequence=['crimson', 'grey'],
                )
...
fig.write_html(fr'C:/Users/jborr/OneDrive/Noven/SHW/Waterfront/Johan histogram non-zero.html')
fig.show('browser')
#%%% 📊zero histogram
dfFig = dfNew[(dfNew['diff']==0)
              & ((dfNew['EPI'].str.contains('allAps') | (dfNew['EPI'].str.contains('Waterfront CME'))))
...
fig = px.histogram(dfFig,
                ...
                histnorm = 'probability',
                barmode = 'overlay',
                title = '<b>🚿Waterfront: SHW Flow = 0 m³/hour</b><br><i>Taking a random null flow, what are the chances of belonging to a specific hour?'+
                f'<br>{dfFig.shape[0]} datapoints - {dfFig["t"].dt.date.min()} to {dfFig["t"].dt.date.max()}',
                marginal = 'box',
...
fig.write_html(fr'C:/Users/jborr/OneDrive/Noven/SHW/Waterfront/Johan histogram zero.html')
fig.show('browser')
