...
instances = [    
    'Algemeen ALSB',  # Net
    'PV',
    'Elektriciteitsteller WKK', 
    'Elektriciteitsteller Warmtepomp 1',  
    'Elektriciteitsteller Warmtepomp 2',  
    'Elektriciteitsteller Warmtepomp 3',  
    'Elektriciteitsteller Warmtepomp 4', 
    'Elektriciteitsteller Warmtepomp 5',  
    'Elektriciteitsteller Warmtepomp 6', 
    ]
...
# To wide and rename
for i in dfj['I'].unique():
    aux = dfj[dfj['I'] == i].resample('1h').first().fillna(method='ffill')
    aux.rename(columns={'ave': i}, inplace=True)  
    wide = pd.concat([wide, aux[i]], axis = 1)

wide['prod'] = wide['PV'] + wide['Elektriciteitsteller WKK']
wide['cons'] = wide.filter(regex='Elektriciteitsteller Warmtepomp').sum(axis=1)
wide['aux'] = wide['prod']- wide['cons']
wide['']
...
#%%
counter = 'VME Arkenvest Parkrand'
...
out.to_csv(f"C:/Users/jborr/OneDrive/Noven/Repos/python-scripts-dataminer/ZeroFriction/{counter}/{counter}.csv", sep=";", decimal=",", index=False)
